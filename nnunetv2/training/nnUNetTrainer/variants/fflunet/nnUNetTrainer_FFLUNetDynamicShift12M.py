...
from nnunetv2.training.nnUNetTrainer.variants.fflunet.blocks13M import (
...
class FFLUNetDynamicWindowShift12M(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x1 = self.m1(x, w=FFLUNetDynamicWindowShift12M.get_dynamic_shifts(x))
        x = self.d1(x)

        x = x2 = self.m2(x, w=FFLUNetDynamicWindowShift12M.get_dynamic_shifts(x))
        x = self.d2(x)

        x = x3 = self.m3(x, w=FFLUNetDynamicWindowShift12M.get_dynamic_shifts(x))
        x = self.d3(x)

        x = x4 = self.m4(x, w=FFLUNetDynamicWindowShift12M.get_dynamic_shifts(x))
        x = self.d4(x)
...
class nnUNetTrainer_FFLUNetDynamicShift12M(nnUNetTrainerNoDeepSupervision):
    @staticmethod
    def build_network_architecture(
        ...
        return FFLUNetDynamicWindowShift12M(
