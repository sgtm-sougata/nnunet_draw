...
class FFLUNetDynamicWindowShift4Layers(nn.Module):
    ...
    def forward(self, x):
        ...
        x = x1 = self.m1(x, w=FFLUNetDynamicWindowShift4Layers.get_dynamic_shifts(x))
        x = self.d1(x)

        x = x2 = self.m2(x, w=FFLUNetDynamicWindowShift4Layers.get_dynamic_shifts(x))
        x = self.d2(x)

        x = x3 = self.m3(x, w=FFLUNetDynamicWindowShift4Layers.get_dynamic_shifts(x))
        x = self.d3(x)
...
class nnUNetTrainer_FFLUNetDynamicShift4Layers(nnUNetTrainerNoDeepSupervision):
    @staticmethod
    def build_network_architecture(
        ...
        return FFLUNetDynamicWindowShift4Layers(
